...
class LaneDetection(Node):
    def __init__(self):
        ...
        self.Blue_Hue_low = self.get_parameter('Blue_Hue_low').value
        self.Blue_Hue_high = self.get_parameter('Blue_Hue_high').value
        self.Blue_Saturation_low = self.get_parameter('Blue_Saturation_low').value
        self.Blue_Saturation_high = self.get_parameter('Blue_Saturation_high').value
        self.Blue_Value_low = self.get_parameter('Blue_Value_low').value
        self.Blue_Value_high = self.get_parameter('Blue_Value_high').value
    ...
    def locate_centroid(self, data):
        ...
        lowerblue  = np.array([self.Blue_Hue_low, self.Blue_Saturation_low, self.Blue_Value_low])
        upperblue  = np.array([self.Blue_Hue_high, self.Blue_Saturation_high, self.Blue_Value_high])
        ...
        # Finding contours for the blue lane
        bitwise_mask_blue = cv2.bitwise_and(hsv, hsv, mask=maskblue)
        gray_blue = cv2.cvtColor(bitwise_mask_blue, cv2.COLOR_BGR2GRAY)
        (dummy, blackAndWhiteImageBlue) = cv2.threshold(gray_blue, self.gray_lower, gray_upper, cv2.THRESH_BINARY)

        # Removing noise for blue mask
        blurred_blue = cv2.blur(blackAndWhiteImageBlue, (self.kernal_size, self.kernal_size))
        ...
        (dummy, blackAndWhiteImageBlue) = cv2.threshold(dilation_blue, self.gray_lower, gray_upper, cv2.THRESH_BINARY)
        contours_blue, _ = cv2.findContours(blackAndWhiteImageBlue, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        ...
        for contour in contours[:self.number_of_lines]:
            ...
            if self.min_width < w < self.max_width:
                try:
                    ...
                    pass
        for contour in contours_blue[:self.number_of_lines]:
            [x, y], [w, h], phi = cv2.minAreaRect(contour)
            rect = cv2.minAreaRect(contour)
            if self.min_width < w < self.max_width:
                try:
                    box = cv2.boxPoints(rect)
                    box = np.int0(box)
                    img = cv2.drawContours(img, [box], 0, (255, 0, 0), 3)  # Blue Contours in blue
                    m = cv2.moments(contour)
                    cx = int(m['m10'] / m['m00'])
                    cy = int(m['m01'] / m['m00'])
                    cx_list_blue.append(cx)
                    cy_list_blue.append(cy)
                    cv2.circle(img, (cx, cy), 7, (255, 0, 0), -1)  # Blue centroids in blue
                except ZeroDivisionError:
                    pass
...
# Further image processing to determine optimal steering value
        try:
            ...
            elif len(cx_list_blue) > 0:  # Blue lane detected → Drift mode                
                counter = 0
                if counter % 2 == 0:
                       self.centroid_error.data = 999.0  # right Drift mode
                       count = count + 1
                else:
                    self.centroid_error.data = 998.0  # left  Drift mode
                    count = count + 1
                # self.centroid_error.data = 999.0
                self.centroid_error_publisher.publish(self.centroid_error)
                self.get_logger().info("Blue lane detected! Publishing stop signal (999/998).")
                mid_x, mid_y = cx_list_blue[0], cy_list_blue[0]
                img = cv2.line(img, (cam_center_line_x, mid_y), (mid_x, mid_y), (255, 0, 0), 4)  # Blue line for drift tracking
                self.centroid_error_publisher.publish(self.centroid_error)
                self.get_logger().info("No yellow detected. Blue Lane Found! Initiating Drift Mode...")
 
            # When Nothing was found
            # When Nothing was found
            else:
                self.get_logger().info("No lane detected! Forcing right turn for 4 seconds.") 
                # clean slate
            error_list = [-2] * self.number_of_lines
            cx_list = []
            cy_list = []
        except ValueError:
            pass
