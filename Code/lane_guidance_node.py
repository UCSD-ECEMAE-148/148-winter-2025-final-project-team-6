...
class PathPlanner(Node):
    ...
    def controller(self, data):
        ...
        self.ek = data.data
        if self.ek == 999:  # Special value meaning "no lane detected"
            self.get_logger().info("No lane detected! Setting throttle to 0.")
            self.twist_cmd.linear.x = 0.1 # ✅ Stop moving forward
            self.twist_cmd.angular.z = 1.5  # Optional: Stop turning
            self.twist_publisher.publish(self.twist_cmd)
            return  # Stop further calculations
        if self.ek == 998:  # Special value meaning "no lane detected"
            self.get_logger().info("No lane detected! Setting throttle to 0.")
            self.twist_cmd.linear.x = 0.1 # ✅ Stop moving forward
            self.twist_cmd.angular.z = -1.5  # Optional: Stop turning
            self.twist_publisher.publish(self.twist_cmd)
